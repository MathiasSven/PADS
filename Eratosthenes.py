...
def FermiDirac():
    ...
    PrimeIterator = primes()
    SquareHeap = []
    x = next(PrimeIterator)
    q = x*x
    p = next(PrimeIterator)
    while True:
        # Invariants:
        #   x is the next output
        #   p and q are the smallest prime and square after x
        #   the rest of the primes will be generated by PrimeIterator
        #   all squares of output+x are in output+x+SquareHeap
        yield x
        if p < q: x,p = p,next(PrimeIterator)
        else: x,q = q,heapq.heappop(SquareHeap)
        heapq.heappush(SquareHeap,x*x)
